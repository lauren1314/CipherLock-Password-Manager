...
# Create an empty set to store registered usernames
registered_users = {}
...
# Helps to check if the username is already registered
    if username in registered_users:
        feedback_label.config(text="Username already registered. Please choose a different username.")
    else:
        strength_score = calculate_password_strength(password)
        hashed_password = password
        encrypted_password = custom_cipher_encrypt(hashed_password)

        # Update the dictionary with username and encrypted password
        registered_users[username] = encrypted_password
...
#retrieved from this file
    ...
    if stored_password is not None:
        feedback_label.config(text=f"Stored Password: {stored_password}", font=("TkDefaultFont", 12, "bold"), fg="red")
    else:
...
check_password_button.grid(row=5, column=0, columnspan=2, pady=5)
...
generate_password_button.grid(row=7, column=0, columnspan=2, pady=5)

feedback_label = tk.Label(window, text="")
feedback_label.grid(row=8, column=0, columnspan=2)
